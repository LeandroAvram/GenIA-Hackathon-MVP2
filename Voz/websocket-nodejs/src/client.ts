...
export class NovaSonicBidirectionalStreamClient {
  ...
  private knowledgeBaseId = "3FONOJH7XV";
  ...
  private async processToolUse(toolName: string, toolUseContent: object): Promise<Object> {
    ...
    switch (tool) {
      case "knowledgebasetool":
        console.log(`knowledge base tool - searching for: ${JSON.stringify(toolUseContent)}`);
        const kbContent = await this.parseToolUseContentForKB(toolUseContent);
        if (!kbContent?.query) {
          console.error('No query found in tool content:', toolUseContent);
          throw new Error('Query is required for knowledge base search');
        }
        console.log(`Searching knowledge base with query: ${kbContent.query}`);
        const result = await this.searchKnowledgeBase(kbContent.query);
  ...
  private async parseToolUseContentForKB(toolUseContent: any): Promise<{ query: string } | null> {
    try {
      if (toolUseContent && typeof toolUseContent.content === 'string') {
        const parsedContent = JSON.parse(toolUseContent.content);
        return {
      ...
      console.error("Failed to parse knowledge base tool content:", error);
      return null;
    }
  }

  private async searchKnowledgeBase(query: string): Promise<Record<string, any>> {
    try {
      ...
      console.error(`Error searching knowledge base: ${error instanceof Error ? error.message : String(error)}`, error);
      throw error;
    }
  }
          ...
          toolConfiguration: {
            ...
            },
            {
              toolSpec: {
                ...
                description: "Buscar en la base de conocimiento información relevante para responder preguntas del usuario. USAR SIEMPRE antes de responder.",
                inputSchema: {
